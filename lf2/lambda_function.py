...
def search_elastic_search(labels):
    ...
    url = 'https://search-photos-seypxxu7hgwsmp4d3nqhcnd7oy.us-east-1.es.amazonaws.com/photos/_search?q='
    
    resp = []
    for label in labels:
        if (label is not None) and label != '':
            url2 = url+label
            resp.append(requests.get(url2, auth=('cloudproj', 'Cloud@123')).json())
    print ("RESPONSE" , resp)
    output = []
    for r in resp:
        if 'hits' in r:
